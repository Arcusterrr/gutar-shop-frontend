...
export const HorizontalScrolling: FC<HorizontalScrollingProps> = ({
  ...
  useEffect(() => {
    if (null !== scrollRef.current && null !== scrollListRef.current) {
      ...
      const setTimerLoop = () => {
        timerLoopa = (setTimeout(function inner() {
          if(isDragging) return;

          if (prevTranslate <= -((scrollListRef.current?.offsetWidth ?? 0) + (scrollRef.current?.offsetWidth ?? 0))) {
            currentTranslate = 0;
            setSliderPosition();
            prevTranslate = currentTranslate;
          } else {
          ...
          timerLoopa = setTimeout(inner, 1000) as any;
        }, 1000)) as any;
      }
